...
并使用来源钩子不接受的尾注。09-10 修正规则时没有取得历史迁移授权，因此当时保留原历史。
09-11 开发者明确要求修改 Git 历史后，按规范的授权迁移流程备份并修正；
旧消息、拆分范围与新旧节点关系见[历史迁移记录](git_history_repair_20260911.md)，不将修复后的消息冒称为原始提交消息。
...
协作摘要的范围说明已按来源“需要定位归属时”表述对齐。这次来源复核本身没有改写历史；
之后获明确授权的历史迁移单独记录在[迁移说明](git_history_repair_20260911.md)。
