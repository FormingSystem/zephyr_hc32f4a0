...
第一次实际操作，直接进入 [venv 手把手实训与应用测试模板](python/venv/labs/README.md) 或 [west 手把手实训与工作区验收模板](west/labs/README.md)。每一步给出目的、当前位置、命令与参数解释、状态变化、预期结果和失败恢复；使用全新练习目录，无须先运行维护者的自动回归。下面的大纲用于按机制深入学习。
