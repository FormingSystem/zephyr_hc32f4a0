...
前几章解决了本地怎么操作，本章把此前的操作接成一次交付：怎样让别人取得相同结果。我们先选择由谁维护配套声明，再把检查、发布依赖、接受清单变更和新工作区验证连成一个过程，最后独立添加一个仓库，检验能否把工具迁移到自己的工程。
...
本节沿用原 textbook-01 工作区：app/arithmetic 已获取，guide 默认停用，inventory 已注册。前四章的版本、分组和扩展是本章已知条件；不会再引入新的工具。本练习只增加本机数据，不推送远端。
...
完成本章后用 `git -C manifest diff` 审查清单变化，保存有价值的实验。再次练习时从第一章换一个新实验名，沿同一条路线推进；不需要另找一份综合手册。要清理时，只处理自己生成并核对过的实验目录。

## 5.6 把这次配套写成可以重复的检查

到现在，每个结论都能手工核对。交付前还可以把刚才检查过的关系写成一个只读程序，减少漏项。这不是提前要求你理解一个庞大的测试框架；下面的完整代码只组合本专题已有命令和预期结果。
...
output 用 subprocess 启动命令并取回输出，非零退出会立即报告异常；strip 去掉末尾换行。assert 表达本次组合必须成立的条件，条件不满足会给出 AssertionError 并非零退出。这里直接用普通 python 运行，不加会禁用断言的 -O。

expected 明确本次接受的三个项目版本；west list 提供声明，Git 提供实际 HEAD、同步基线和标签对应提交。三个提交相同，才说明这次检出符合本例的固定组合。最后再运行应用并检查新版检查表内容，避免只验证“文件存在”。这个模板不声称适用于任意清单，也不代替真实产品业务测试。
