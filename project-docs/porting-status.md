...
### 2026-09-21：将环境课程重编为单元内实验教材

修订 learning 的准备章和十五篇专题正文，把源码、配置、操作、解释与失败恢复放回对应单元；实验目录仅保存材料，不再要求初阶读者跳转综合手册。venv 第一章保留读者的项目隔离需求并修正 PATH 表述，west 首章只启用必要的两个仓库，CMake 模块与 Kconfig 入门移除对后续测试或设备树知识的提前依赖。

补全主机新增限幅文件、模块条件编译、编辑器完整工作区、设备树正反输入与 Twister 修改练习；新增 west 交付检查源码，能抓住实际检出偏离清单。项目 Python/west 页面同步阅读入口，仍只维护当前工程事实。

新版 venv/west 正文顺序实跑及独立材料回归通过；主机 CMake/CTest/GDB、模块开启关闭、HC32 配置和编译、非法设备树拒绝与恢复、QEMU 正反场景通过。工程 49 项工具检查与 pyOCD 离线检查、固件与源码审计通过。详细方法和边界见 learning/VALIDATION.md 与 BUILD_BOARD_VALIDATION.md。未操作 VS Code 图形调试、menuconfig 交互或真实硬件，不把模拟结果列为板卡验收。
